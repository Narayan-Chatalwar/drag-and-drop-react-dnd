...
class Cart extends React.Component {
  constructor(props) {
    super(props);
    this.state = { selectedFields: [], lastSelectedIndex: -1, finalList: [] };
  ...
  handleItemSelection(index, cmdKey, shiftKey) {
    ...
    if (!cmdKey && !shiftKey) {
      ...
      const foundIndex = this.state.selectedFields.findIndex(
        (f) => f === field
      );
    ...
    localStorage.setItem("selecteditems", JSON.stringify(finalList));
  }

  render() {
    const items = this.props.fields.map((field, index) => (
      <Item
        name={field}
        key={index}
    ...
    return this.props.connectDropTarget(
      <div style={styles.content}> {items} </div>
    );
  }
}
